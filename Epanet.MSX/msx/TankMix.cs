...
public class TankMix {
    ...
    public void  MSXtank_mix1(int i, double vIn, double[] cIn, long dt)
    {
        ...
        k = MSX.Nobjects[(int)EnumTypes.ObjectTypes.LINK] + i;
        ...
        if ( vIn > 0.0 )
            chemical.MSXchem_equil(EnumTypes.ObjectTypes.NODE, MSX.Tank[i].getC());
    }
    ...
    public void  MSXtank_mix2(int i, double vIn, double[] cIn, long dt)
    {
        ...
        k = MSX.Nobjects[(int)EnumTypes.ObjectTypes.LINK] + i;
        seg1 = MSX.Segments[k].First.Value;//get(0);
        seg2 = MSX.Segments[k].Last.Value;//get(MSX.Segments[k].size()-1);
    ...
    */
    ...
    public void  MSXtank_mix3(int i, double vIn, double[] cIn, long dt)
    {
        ...
        vSum = 0.0;
        for (m=1; m<=MSX.Nobjects[(int)EnumTypes.ObjectTypes.SPECIES]; m++) MSX.C1[m] = 0.0;

        // Withdraw flow from first segment
        while (vOut > 0.0)
        {
            if (MSX.Segments[k].Count == 0) break;

            // --- get volume of current first segment
            seg = MSX.Segments[k].First.Value;//[0];

            vSeg = seg.getV();
            vSeg = Math.Min(vSeg, vOut);
            if ( seg ==  MSX.Segments[k].Last.Value ) vSeg = vOut;  //[MSX.Segments[k].size()-1]       //TODO pode ser simplificado para getSize()==1
        ...
        for (m=1; m<=MSX.Nobjects[(int)EnumTypes.ObjectTypes.SPECIES]; m++)
        {
            ...
            else            MSX.Tank[i].getC()[m] = MSX.Segments[k].First.Value.getC()[m]; //MSX.Segments[k][0].getC()[m];
        }
    ...
    public void  MSXtank_mix4(int i, double vIn, double[] cIn, long dt)
    {
        ...
        if ( vNet > 0.0 )
        {
            ...
            seg = null;
            if(MSX.Segments[k].Count>0)
                seg = MSX.Segments[k].Last.Value;//[MSX.Segments[k].size()-1];
            ...
            for (m=1; m<=MSX.Nobjects[(int)EnumTypes.ObjectTypes.SPECIES]; m++) MSX.Tank[i].getC()[m] = cIn[m];
        ...
        else if (vNet < 0.0)
        {
            ...
            while (vNet > 0.0)
            {
                ...
                if ( seg == MSX.Segments[k].First.Value ) vSeg = vNet;
                ...
                {
                    MSX.Segments[k].RemoveLast();//remove(MSX.Segments[k].size()-1);
