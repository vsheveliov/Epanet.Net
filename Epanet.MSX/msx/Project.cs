...
namespace Epanet.MSX {
    ...
    public class Project {
        ...
        public int MSXproj_addObject(ObjectTypes type, string id, int n) {
            int result = 0;
            string newId = id;
...
// --- insert object's ID into the hash table for that type of object
        ...
        public int MSXproj_findObject(ObjectTypes type, string id) {
            int val;
        ...
        private ErrorCodeType ConvertUnits() {
            ...
            if (this.msx.Unitsflag == UnitSystemType.US) {
                this.msx.Ucf[(int)UnitsType.LENGTH_UNITS] = 1.0;
                this.msx.Ucf[(int)UnitsType.DIAM_UNITS] = 12.0;
                this.msx.Ucf[(int)UnitsType.VOL_UNITS] = 1.0;
            }
            else {
                this.msx.Ucf[(int)UnitsType.LENGTH_UNITS] = Constants.MperFT;
                this.msx.Ucf[(int)UnitsType.DIAM_UNITS] = 1000.0 * Constants.MperFT;
                this.msx.Ucf[(int)UnitsType.VOL_UNITS] = Constants.M3perFT3;
            }

            // Conversion for surface area
            this.msx.Ucf[(int)UnitsType.AREA_UNITS] = 1.0;
            switch (this.msx.AreaUnits) {
            case AreaUnitsType.M2:
                this.msx.Ucf[(int)UnitsType.AREA_UNITS] = Constants.M2perFT2;
                break;
            case AreaUnitsType.CM2:
                this.msx.Ucf[(int)UnitsType.AREA_UNITS] = Constants.CM2perFT2;
                break;
            }

            // Conversion for flow rate
            this.msx.Ucf[(int)UnitsType.FLOW_UNITS] = fcf[(int)this.msx.Flowflag];
            this.msx.Ucf[(int)UnitsType.CONC_UNITS] = Constants.LperFT3;

            // Conversion for reaction rate time
            this.msx.Ucf[(int)UnitsType.RATE_UNITS] = rcf[(int)this.msx.RateUnits];

            // Convert pipe diameter & length
            for (int i = 1; i <= this.msx.Nobjects[(int)ObjectTypes.LINK]; i++) {
                this.msx.Link[i].Diam = this.msx.Link[i].Diam / this.msx.Ucf[(int)UnitsType.DIAM_UNITS];
                this.msx.Link[i].Len = this.msx.Link[i].Len / this.msx.Ucf[(int)UnitsType.LENGTH_UNITS];
            }

            // Convert initial tank volumes
            for (int i = 1; i <= this.msx.Nobjects[(int)ObjectTypes.TANK]; i++) {
                this.msx.Tank[i].V0 = this.msx.Tank[i].V0 / this.msx.Ucf[(int)UnitsType.VOL_UNITS];
                this.msx.Tank[i].VMix = this.msx.Tank[i].VMix / this.msx.Ucf[(int)UnitsType.VOL_UNITS];
            }
        ...
        private ErrorCodeType CreateObjects() {
            // Create nodes, links, & tanks
            this.msx.Node = new Node[this.msx.Nobjects[(int)ObjectTypes.NODE] + 1];
            this.msx.Link = new Link[this.msx.Nobjects[(int)ObjectTypes.LINK] + 1];
            this.msx.Tank = new Tank[this.msx.Nobjects[(int)ObjectTypes.TANK] + 1];

            // Create species, terms, parameters, constants & time patterns
            this.msx.Species = new Species[this.msx.Nobjects[(int)ObjectTypes.SPECIES] + 1];
            this.msx.Term = new Term[this.msx.Nobjects[(int)ObjectTypes.TERM] + 1];
            this.msx.Param = new Param[this.msx.Nobjects[(int)ObjectTypes.PARAMETER] + 1];
            this.msx.Const = new Const[this.msx.Nobjects[(int)ObjectTypes.CONSTANT] + 1];
            this.msx.Pattern = new Pattern[this.msx.Nobjects[(int)ObjectTypes.PATTERN] + 1];

            for (int i = 0; i <= this.msx.Nobjects[(int)ObjectTypes.CONSTANT]; i++)
                this.msx.Const[i] = new Const();

            // Create arrays for demands, heads, & flows
            this.msx.D = new float[this.msx.Nobjects[(int)ObjectTypes.NODE] + 1];
            this.msx.H = new float[this.msx.Nobjects[(int)ObjectTypes.NODE] + 1];
            this.msx.Q = new float[this.msx.Nobjects[(int)ObjectTypes.LINK] + 1];

            // create arrays for current & initial concen. of each species for each node
            this.msx.C0 = new double[this.msx.Nobjects[(int)ObjectTypes.SPECIES] + 1];
            for (int i = 1; i <= this.msx.Nobjects[(int)ObjectTypes.NODE]; i++) {
                this.msx.Node[i] = new Node(this.msx.Nobjects[(int)ObjectTypes.SPECIES] + 1);
            ...
            for (int i = 1; i <= this.msx.Nobjects[(int)ObjectTypes.LINK]; i++) {
                this.msx.Link[i] = new Link(
                    this.msx.Nobjects[(int)ObjectTypes.SPECIES] + 1,
                    this.msx.Nobjects[(int)ObjectTypes.PARAMETER] + 1);
            ...
            for (int i = 1; i <= this.msx.Nobjects[(int)ObjectTypes.PATTERN]; i++) {
            ...
            for (int i = 1; i <= this.msx.Nobjects[(int)ObjectTypes.TERM]; i++) {
        ...
        private ErrorCodeType CreateHashTables() {


            // create a hash table for each type of object
            this.htable = new Dictionary<string, int>[(int)ObjectTypes.MAX_OBJECTS];

            for (int j = 0; j < (int)ObjectTypes.MAX_OBJECTS; j++) {
