...
public class SparseMatrix {
     ...
     */
    private void buildlists( List<AdjItem>[] adjlist,List<SimulationNode> nodes, List<SimulationLink> links, bool paraflag) {
    ...
    private void xparalinks(List<AdjItem>[] adjlist)
    {
        ...
        {
            adjlist[i].RemoveAll(x => x.getNode() == 0);
        }
    }
     ...
     */
    private bool linked(List<AdjItem>[] adjlist,int i, int j) {
        foreach (AdjItem alink  in  adjlist[i])
            if (alink.getNode() == j)
                return true;
        return false;
     ...
     * @param n
     */
    private void addlink(List<AdjItem>[] adjList,int i, int j, int n) {
