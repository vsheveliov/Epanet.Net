...
namespace Epanet {
    ...
    public class SampleOOPNetworkDyn2 {
        public static void main(string[] args) {
            ...
            for (int i = 1; i < 7; i++) {
                network.Nodes.Add(node[i]);
            }

            for (int i = 0; i < 8; i++) {
                network.Links.Add(pipe[i]);
            }
            ...
            new DynamicSimulation(network, log).Simulate();
        }

        class DynamicSimulation:HydraulicSim {

            public DynamicSimulation(Network.Network net, TraceSource log):base(net, log) { }
