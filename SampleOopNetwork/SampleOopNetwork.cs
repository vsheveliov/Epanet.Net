...
namespace Epanet.Samples {
    ...
    public class SampleOopNetwork {
        public static void Main(string[] args) {
            ...
            //Prepare Network
            TraceSource log = new TraceSource(typeof(SampleOopNetwork).FullName, SourceLevels.All);
            ...
            for (TimeSpan time = net.RStart; time <= net.Duration; time += net.RStep) {
                ...
                foreach (Node inode in net.Nodes)
                    Console.Write("{0:F2}\t", step.GetNodeHead(i++, null));
